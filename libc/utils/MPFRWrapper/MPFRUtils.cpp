  ...
  ulp_as_mpfr_number(T input) {
    ...
    if (thisAsT * input < 0 || thisExponent == inputExponent) {
      ...
      mpfr_mul_2si(inputMPFR.value, inputMPFR.value,
                   -thisExponent + int(fputil::MantissaWidth<T>::VALUE),// FLOAT80 MANTISSA
                   MPFR_RNDN);
      return inputMPFR;
    }
    ...
    mpfr_mul_2si(minMPFR.value, minMPFR.value,
                 -minExponent + int(fputil::MantissaWidth<T>::VALUE),// FLOAT80 MANTISSA
                 MPFR_RNDN);

    mpfr_sub(maxMPFR.value, maxMPFR.value, pivot.value, MPFR_RNDN);
    mpfr_mul_2si(maxMPFR.value, maxMPFR.value,
                 -maxExponent + int(fputil::MantissaWidth<T>::VALUE),// FLOAT80 MANTISSA
                 MPFR_RNDN);
